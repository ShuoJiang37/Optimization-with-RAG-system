...
int main(){
  ...
  Bub(trump1,n);
  PP(trump1,n);
  stable=stablecheck(trump,n,trump1);
  printf("Stable\n");
  /*selection sort*/
  Sel(trump2,n);
  PP(trump2,n);
...
int stablecheck(donald *in, int n,donald *out){
  ...
  for(i=0;i<n;i++){
    for(j=i+1;j<n;j++){
      for(a=0;a<n;a++){
	for(b=a+1;b<n;b++){
	  if(in[i].number==in[j].number && in[i].number==out[a].number && in[j].number==out[b].number && in[i].c==in[j].c && in[i].c==out[a].c && in[j].c==out[b].c){
	    return 0;
	  }
	}
      }
